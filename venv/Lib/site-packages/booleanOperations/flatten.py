...
class InputSegment:
    ...
    def __init__(self, points=None, previousOnCurve=None, willBeReversed=False):
        ...
        if self.segmentType == "curve":
            ...
            elif previousOnCurve[0] == points[0].coordinates[0] == points[1].coordinates[0] == points[-1].coordinates[
                0]:
                oncurve = points[-1]
                oncurve.segmentType = "line"
                self.points = points = [oncurve]
            elif previousOnCurve[1] == points[0].coordinates[1] == points[1].coordinates[1] == points[-1].coordinates[
                1]:
                oncurve = points[-1]
                oncurve.segmentType = "line"
                self.points = points = [oncurve]
...
class OutputContour:
            ...
            for segment in newSegments:
                ...
                if len(segmentedFlatPoints) > 1 and len(segmentedFlatPoints[0]) == 1:
                    ...
                    if fp in flatInputPoints and lp in flatInputPoints:
                        ...
                        if (firstInputSegment.segmentType == reversedFirstInputSegment.segmentType == "curve") or (
                                lastInputSegment.segmentType == reversedLastInputSegment.segmentType == "curve"):
                ...
                for flatSegment in segmentedFlatPoints:
                    ...
                    if ep is None and ((fp is None) or (len(flatSegment) == 2)):
                        ...
                        if ep is not None and ep in inputSegment.flat:
                            ...
                            if flatSegment[0] == reversedInputSegment.flat[0] and flatSegment[-1] == \
                                    reversedInputSegment.flat[-1]:
                                inputSegment = reversedInputSegment
                            elif flatSegment[0] in intersectionPoints and flatSegment[-1] == reversedInputSegment.flat[
                                -1]:
                                inputSegment = reversedInputSegment
                            elif flatSegment[-1] in intersectionPoints and flatSegment[0] == reversedInputSegment.flat[
                                0]:
                                inputSegment = reversedInputSegment
                            canDoFastLine = False
                    ...
                    if (len(flatSegment) == 1 or inputSegment is None) and canDoFastLine:
                        ...
                        for p in flatSegment:
                            ...
                            convertedSegments.append(
                                OutputPoint(coordinates=previousIntersectionPoint, segmentType="line", kwargs=kwargs,
                                            **pointInfo))
                        continue
                    tValues = None
    ...
    def drawPoints(self, pointPen):
        ...
        for point in points:
            if previousPointCoordinates is not None and point.segmentType and tuple(
                    point.coordinates) == previousPointCoordinates:
                continue
            kwargs = {}
...
def _tValueForPointOnQuadCurve(point, pts, isHorizontal=0):
    ...
    for index, (pt1, pt2) in enumerate(quadSegments):
        ...
        subSolutions = bezierTools.solveQuadratic(a[isHorizontal], b[isHorizontal],
