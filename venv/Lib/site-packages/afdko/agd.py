    ...
    def parse(self, intext):
        ...
        for e in ee:
            ...
            if k == 'uni':
                ...
                self.fin = re_name.search(x).group()  # final name
            elif k == 'ali':
                for a in re_name.findall(x): self.ali.append(a)  # name aliases
            elif k == 'sub':
                self.sub = x.split()  # substitutions
            elif k == 'set':
                self.set.extend(x.split())  # glyph sets
            elif k == 'min':
                self.min = re_name.search(x).group()  # minuscule
            elif k == 'maj':
                self.maj = re_name.search(x).group()  # majuscule
            elif k == 'cmp':
                self.cmp = x  # components
            else:
                self.other[k] = x  # any unknown entry key
...
class dictionary:
    ...
    def glyph(self, n):
        if not n:
            ...
            return self.glyphs[self.index[n]]  # lookup by glyph name
        elif n in self.unicode:
            return self.glyphs[self.unicode[n]]  # lookup by Unicode value
        else:
            return None
    ...
    def add(self, g, priority=1):
        ...
        if t:
            ...
            m.append("Adding new glyph: %s" % g.name)

        # Add the glyph to the dictionary:
        if addplace is None:
            self.list.append(g.name)
        else:
            self.list.insert(addplace, g.name)  # replace the old glyph with the new glyph in the order list
