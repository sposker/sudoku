...
def getOptions(params):
    ...
    while i < numOptions:
        ...
        if arg == "-h":
            ...
            if not hasattr(params, name):
                ...
                if name.endswith("RGB"):
                    ...
                    if match:
                        ...
                        print(
                            "Error: parameter %s must take an RGB triplet, enclosed in quotes, such as '( 1.0, 0, 0)' for red." % name)
                        raise OptionParseError
                elif name == "pageSize":
                    ...
                    if match:
                        ...
                        print(
                            "Error: parameter %s must take an (x,y) pair, enclosed in quotes, such as ' (612.0, 792.0)'." % name)
                        raise OptionParseError
            ...
            exec("params." + kDrawPointTag + "PointLabels = 0")
            exec(
                "params." + kShowMetaTag + "Outline = 0")  # shows filled outline in  lower right of meta data area under glyph outline.
            exec("params." + kShowMetaTag + "Name = 1")
            exec("params." + kShowMetaTag + "BBox = 0")
            exec("params." + kShowMetaTag + "SideBearings = 1")
            exec("params." + kShowMetaTag + "V_SideBearings = 0")
            ...
            if sizeString[0] == "-":
                raise OptionParseError(
                    "Option Error: it looks like the first item in the waterfall size list following '-wfr' is another option.")
            ...
            params.emboxColorRGB = params.baselineColorRGB = params.xAdvanceColorRGB = params.contourLabelColorRGB = params.pointLabelColorRGB = params.MetaDataGlyphColorRGB = (
                0, 0, 0)
            ...
            if rt_filePath[0] == "-":
                raise OptionParseError(
                    "Option Error: it looks like the the layout file following '-gf' is another option.")
            ...
            try:
                ...
                raise OptionParseError(
                    "Option Error: in \" -select_hints %s\,  one of the indices in the argument list is not an integer." %
                    sys.argv[i])
...
def openFile(path, txPath):
    ...
    if "CIDFontName" in report:
        ...
        if len(head) < 10:
            raise FontError(
                "Error: font file was zero size: may be a resource fork font, which this program does not process. <%s>." % path)
