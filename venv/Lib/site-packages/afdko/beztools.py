        ...
        if op == "vlineto":
            ...
            if (dy3 == 0 == dy4):
                if (dy1 == dy6 == 0) and (dy2 == -dy5):
                    newT2List.append([[dx1, dx2, dy2, dx3, dx4, dx5, dx6],
                                      "hflex"])  # the device pixel threshold is always 50 , when coming back from AC.
                    noFlex = 0
                else:
...
class CFFFontData:
    ...
    def saveChanges(self):
        ...
        if fontType == 0:  # OTF
            if overwriteOriginal:
                ...
                if os.path.exists(inputPath):
                    try:
                        ...
                        self.logMsg(
                            "\t%s" % (traceback.format_exception_only(sys.exc_info()[0], sys.exc_info()[1])[-1]))
                        self.logMsg(
            ...
            if fontType == 1:  # CFF
                ...
                if "fatal" in report:
                    raise IOError(
                        "Failed to convert hinted font temp file with tx %s. Maybe target font font file '%s' is set to read-only. %s" % (
                            tempPath, outFilePath, report))
        ...
        if hasattr(privateDict, "BlueValues"):
            ...
            if numBlueValues < 4:
                if allow_no_blues:
                    ...
                    raise ACFontError(
                        "Error: font must have at least four values in its BlueValues array for AC to work!")
        else:
            if allow_no_blues:
                blueValues = inactiveAlignmentValues
                numBlueValues = len(blueValues)
    ...
    def getfdInfo(self, fontPSName, inputPath, allow_no_blues, noFlex, vCounterGlyphs, hCounterGlyphs, glyphList,
                  fdIndex=0):
        ...
        fdDict = self.getFontInfo(fontPSName, inputPath, allow_no_blues, noFlex, vCounterGlyphs, hCounterGlyphs,
                                  fdIndex)
