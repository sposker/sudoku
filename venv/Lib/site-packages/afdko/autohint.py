...
def parseCounterHintData(path):
    ...
    for line in lines:
        ...
        if (len(fields) != 2) or (fields[0] not in ["V", "v", "H", "h"]):
            print(
                "\tError: could not process counter hint line '%s' in file %s. Doesn't look like V or H followed by a tab or space, and then a glyph name." % (
                    line, path))
...
def getOptions():
    ...
    while i < numOptions:
        ...
        if arg == "-h":
            ...
            if glyphString[0] == "-":
                raise ACOptionParseError(
                    "Option Error: it looks like the first item in the glyph list following '-g' is another option.")
            ...
            if filePath[0] == "-":
                raise ACOptionParseError(
                    "Option Error: it looks like the the glyph list file following '-gf' is another option.")
            try:
                gf = open(filePath, "rt")
                glyphString = gf.read()
                gf.close()
            ...
            if filePath[0] == "-":
                raise ACOptionParseError(
                    "Option Error: it looks like the the counter hint glyph list file following '-cf' is another option.")
            try:
    ...
    if not os.path.exists(options.inputPath):
        ...
        options.inputPath = options.inputPath.rstrip(
            os.sep)  # might be a UFO font. auto completion in some shells adds a dir separator, which then causes problems with os.path.dirname().
...
def getGlyphNames(glyphTag, fontGlyphList, fontFileName):
    ...
    if prevGID == None:
        if len(rangeList) > 1:
            logMsg("\tWarning: glyph ID <%s> in range %s from glyph selection list option is not in font. <%s>." % (
                rangeList[0], glyphTag, fontFileName))
        else:
            logMsg("\tWarning: glyph ID <%s> from glyph selection list option is not in font. <%s>." % (
                rangeList[0], fontFileName))
        return None
...
def openUFOFile(path, outFilePath, useHashMap):
    ...
    font.requiredHistory.append(
        ufotools.kCheckOutlineName)  # Programs in this list must be run before autohint, if the outlines have been edited.
    return font
...
def hintFile(options):
    ...
    if options.hCounterGlyphs or options.vCounterGlyphs:
        ...
        if missingList:
            logMsg("\tError: glyph named in counter hint list file '%s' are not in font: %s" % (
                options.counterHintFile, missingList))

    #    build alignment zone string
    if (options.printDefaultFDDict):
        logMsg("Showing default FDDict Values:")
        fdDict = fontData.getFontInfo(psName, path, options.allow_no_blues, options.noFlex, options.vCounterGlyphs,
                                      options.hCounterGlyphs)
        parseFontInfoString(str(fdDict))
        fontData.close()
        return

    fdGlyphDict, fontDictList = fontData.getfdInfo(psName, path, options.allow_no_blues, options.noFlex,
    ...
    for name in glyphList:
        ...
        if isCID:  #
            ...
            if not fdIndex == lastFDIndex:
                lastFDIndex = fdIndex
                fdDict = fontData.getFontInfo(psName, path, options.allow_no_blues, options.noFlex,
                                              options.vCounterGlyphs, options.hCounterGlyphs, fdIndex)
                with open(tempFI, "w") as fp:
                    fp.write(fdDict.getFontInfo())
        else:
            if (fdGlyphDict != None):
            ...
            try:
                ...
                if hasHints and not options.rehint:
                    ...
                    if not isNewPlistFile:
                        ...
                        logMsg(
                            "%s Skipping glyph - it has hints, but it is not in the hint info plist file." % aliasName(
                                name))
                        dotCount = 0
                    continue
            if prevACIdentifier and (
                    prevACIdentifier == ACidentifier):  # there is an entry in the plist file and it matches what's in the font.
        ...
        if oldBezString != "" and oldBezString == bezString:
            ...
            command = '"%s" %s%s%s%s -s %s -f "%s" "%s"' % (
                AUTOHINTEXE, verboseArg, suppressEditArg, supressHintSubArg, decimalArg, NEWBEZ_SUFFIX, tempFI, tempBez)
            if options.debug:
                print(command)
            report = fdkutils.runShellCmd(command)
            if report:
        ...
        if not (("ry" in newBezString[:200]) or ("rb" in newBezString[:200]) or ("rm" in newBezString[:200]) or (
                "rv" in newBezString[:200])):
    ...
    if not options.logOnly:
        if anyGlyphChanged:
            ...
            if options.usePlistFile:
                if options.rehint:
                    logMsg("No new hints. All glyphs had hints that matched the hint record file %s." % (
                        fontPlistFilePath))
                if options.hintAll:
                    logMsg(
