    ...
    def addGlyphReport(self, reportString):
        ...
        for line in lines:
            ...
            if key == kCharZone:
                ...
                raise ACFontError(
                    "Error: Found unknown keyword %s in report file for glyph %s." % (key, self.glyphName))
...
def collectStemsFont(path, options):
    ...
    fdGlyphDict, fontDictList = fontData.getfdInfo(psName, path, allow_no_blues, noFlex, vCounterGlyphs, hCounterGlyphs,
                                                   glyphList)
