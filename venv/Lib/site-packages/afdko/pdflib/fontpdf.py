...
class FontPDFParams:
    def __init__(self):
        ...
        # is not in this list.
        self.sortedMethodsDraw = ["EMBox", "Outline", "HHints", "VHints", "BlueZones", "Baseline", "CenterMarks",
                                  "CenterMarksWithBox", "GlyphBox", "XAdvance", "ContourLabels"]
        ...
        exec(
            "self." + kShowMetaTag + "Outline = 1")  # shows filled outline in  lower right of meta data area under glyph outline.
        ...
        # is not in this list.
        self.sortedMethodsMeta = ["Outline", "Name", "BBox", "SideBearings", "V_SideBearings", "Parts", "Paths",
                                  "Hints", "WidthOnly", "HintDir", "RowFont"]
...
class FontPDFGlyph:
    ...
    def draw(self, params, repeat):
        ...
        if self.extraY == None:
            ...
            extraY += (
                              params.pointLabel_LineLength + params.pointLabelSize) * 2  # Need some margin for the labels that stick out below the outlne.
            self.extraY = extraY
        else:
            extraY = self.extraY
        ...
        rt_canvas.transform(scale, 0, 0, scale, 0,
                            0)  # shift the glyph square up from the bottom of the tile by glyphY, and scale by the ratio kGlyphSquare/ emSquare
    ...
    def drawGlyph_EMBox(self, params):
        ...
        rt_canvas.rect(0, baseLine, emSquare,
                       emSquare)  # by definition; the glyph is scaled so the embox is kGlyphSquare pts.
        rt_canvas.restoreState()
    ...
    def drawGlyph_HHints(self, params):
        ...
        for i in list(range(numHints)):
            ...
            if prevHint and (prevHint[1] >= h1):
                ...
                rt_canvas.setFillColorRGB(hintColorOverlapRGB[0] * 0.6, hintColorOverlapRGB[1] * 0.6,
                                          hintColorOverlapRGB[2] * 0.6)
    ...
    def drawGlyph_BlueZones(self, params):
        ...
        rt_canvas.setFillColorRGB(alignmentZoneColorRGB[0] * 0.5, alignmentZoneColorRGB[1] * 0.5,
                                  alignmentZoneColorRGB[2] * 0.5)
        ...
        for i, zone in enumerate(blueZones):
            ...
            if (i % 2) == 0:
                textXPos = blueZoneOffset + kGlyphSquare - (
                        pdfmetrics.stringwidth(textString, params.pointLabelFont) * 0.001 * ptSize)
            else:
                textXPos = x0
    ...
    def drawMeta_BBox(self, params):
        rt_canvas = params.rt_canvas
        bbox = self.BBox
        rt_canvas.drawString(self.cur_x, self.cur_y,
                             "min = %s, %s  max = %s, %s" % (bbox[0], bbox[1], bbox[2], bbox[3]))
    ...
    def drawMeta_Parts(self, params):
        params.rt_canvas.drawString(self.cur_x, self.cur_y, "Parts: m = %s, l = %s, c = %s, total = %s" % (
            self.numMT, self.numLT, self.numCT, self.numMT + self.numLT + self.numCT))
    ...
    def drawMeta_Hints(self, params):
        ...
        params.rt_canvas.drawString(self.cur_x, self.cur_y,
                                    "Hints:  %s horiz: %s, vert: %s" % (numHhints + numVhints, numHhints, numVhints))
    ...
    def drawMeta_WidthOnly(self, params):
        params.rt_canvas.drawString(self.cur_x, self.cur_y,
                                    "Width = %s, Y Advance = %s" % (self.xAdvance, self.yAdvance))
...
def getMetaDataHeight(params, fontYMin):
    if params.rt_metaDataYOffset == None:
        ...
        if params.drawMeta_Outline:
            glyphHeight = (-fontYMin * params.metaDataFilledGlyphSize / float(
                kGlyphSquare)) + params.metaDataFilledGlyphSize  # in pts
...
def setDefaultHPadding(params, init=None):
    ...
    if params.drawGlyph_HHints:
        params.rt_maxHintLabelWidth = pdfmetrics.stringwidth("<999.0, 999.0> 999.0",
                                                             params.pointLabelFont) * 0.001 * params.pointLabelSize
...
def setDefaultVPadding(params, init=None):
    ...
    if params.drawGlyph_VHints:
        params.rt_glyphVPadding += params.vhintYOffset + (
                2 * 1.2 * params.pointLabelSize)  # amount the hints stick out, plus two lines of type.
...
class FontInfo:
    def __init__(self, pdfFont, glyphList):
        ...
        self.Flags = (
                1 << 2)  # mark it as a symbolic font, so that  glyph names outside the PDF Std set will be recognized.
...
def makeFontSetPDF(pdfFontList, params, doProgressBar=True):
    ...
    kPSNameFieldWidth = int(1 + (
            psNameWidth / params.userPtSize))  # The space to leave at the begining of a line for the PS name, in em-spaces.
    kFontIndexWidth = int(1 + (
            indexWidth / params.userPtSize))  # The space to leave at the begining of every line for the font index, in em-spaces.
    ...
    numAcross = int(
        pageWidth / params.userPtSize) - kFontIndexWidth  # -kFontIndexWidth em-spaces is to allow for an initial font index in the line.
...
def makeKernPairPDF(pdfFont, kernOverlapList, params, doProgressBar=True):
    ...
    for entry in kernOverlapList:
        ...
        if cur_x < labelMargin:
            cur_x = labelMargin
        rt_canvas.drawString(cur_x, cur_y, " %s %s kern %s, pixel overlap area %s at %s ppem." % (
            leftName, rightName, kernValue, overlap, overlapPtSize))
        cur_y -= scaledYAdvance
        lineCount += 1
        if ((cur_y) < params.pageBottomMargin):
            pageCount += 1
