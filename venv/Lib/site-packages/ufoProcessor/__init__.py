...
class DesignSpaceProcessor(DesignSpaceDocument):
            ...
            if os.path.exists(path):
                ...
                if existingUFOFormatVersion > self.ufoVersion:
                    self.problems.append(
                        "Can’t overwrite existing UFO%d with UFO%d." % (existingUFOFormatVersion, self.ufoVersion))
                    continue
    ...
    def getInfoMutator(self):
        ...
        bias, self._infoMutator = self.getVariationModel(infoItems, axes=self.serializedAxes,
                                                         bias=self.newDefaultLocation())
        return self._infoMutator
    ...
    def getKerningMutator(self, pairs=None):
        ...
        bias, self._kerningMutator = self.getVariationModel(kerningItems, axes=self.serializedAxes,
                                                            bias=self.newDefaultLocation())
        return self._kerningMutator
        ...
        for i, sourceDescriptor in enumerate(self.sources):
            ...
            if sourceDescriptor.name not in self.fonts:
                if os.path.exists(sourceDescriptor.path):
                    ...
                    self.problems.append("loaded master from %s, layer %s, format %d" % (
                        sourceDescriptor.path, sourceDescriptor.layerName, getUFOVersion(sourceDescriptor.path)))
    ...
    def makeInstance(self, instanceDescriptor,
        ...
        for glyphName in selectedGlyphNames:
            ...
            if masters is not None:
                ...
                bias, glyphMutator = self.getVariationModel(items, axes=self.serializedAxes,
                                                            bias=self.newDefaultLocation())
            try:
