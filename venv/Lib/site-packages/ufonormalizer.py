...
def main(args=None):
    ...
    parser.add_argument("-a", "--all",
                        help="Normalize all files in the UFO. By default, only files modified since the previous normalization will be processed.",
                        action="store_true")
    ...
    parser.add_argument("--float-precision", type=int, default=DEFAULT_FLOAT_PRECISION,
                        help="Round floats to the specified number of decimal places (default is %d). The value -1 means no rounding (i.e. use built-in repr()." % DEFAULT_FLOAT_PRECISION)
...
def normalizeUFO(ufoPath, outputPath=None, onlyModified=True,
    ...
    try:
        ...
        raise UFONormalizerError(
            "Required formatVersion value not properly formatted in metainfo.plist in %s." % ufoPath)
    if formatVersion > 3:
